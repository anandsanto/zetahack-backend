...
store_suffix = ["'s Store for Bell, Books and Candle", "'s butcher shop", "'s stationeries", "'s flower shop", "'s bistro", "'s gray house airbnb"]
phone_prefix = ['+91', '']
#url = "https://zetahack-backend.herokuapp.com/general/api/addseller"
url = "http://127.0.0.1:8000/general/api/addseller"

async def add_seller(session, idnum):
    dat = {}
    name = rd.choice(random_names)
    dat["age"] = rd.randint(18, 100)
    dat["seller_id"] = str(idnum)
    dat["contact_person_first_name"] = name.split()[0]
    dat["contact_person_last_name"] = name.split()[1]
    dat['store_name'] = dat["contact_person_last_name"] + rd.choice(store_suffix)
    dat['phone'] = rd.choice(phone_prefix) + ''.join([str(rd.randint(1, 9)) for i in range(10)])
    dat['store_address'] = "No 4, Viveganandhar Theru, Dubai Kurukku Sandhu, Dubai Main Rd, Dubai"
